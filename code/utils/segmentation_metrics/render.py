...
def plot_metrics(seg_metrics):
    ...
    for i, (sy, sx) in enumerate(ref.bboxes):
        r = patches.Rectangle((sx.start, sy.start), sx.stop - sx.start, sy.stop - sy.start, edgecolor='g',
                              facecolor='None')
        ax.add_patch(r)
        ax.text(sx.start, sy.start, '{}, IoU: {}'.format(i, iou[i]), fontsize=6, color='w')
    for i, (sy, sx) in enumerate(pred.bboxes):
        r = patches.Rectangle((sx.start, sy.start), sx.stop - sx.start, sy.stop - sy.start, edgecolor='m',
                              facecolor='None')
        ax.add_patch(r)
